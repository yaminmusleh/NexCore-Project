...
void Scanner::Init() {
	...
	for (i = 48; i <= 57; ++i) start.set(i, 24);
	start.set(61, 25);
	start.set(33, 26);
	start.set(60, 27);
	start.set(62, 28);
	...
	keywords.set(L"set", 26);
	keywords.set(L"exit", 27);
	keywords.set(L"print", 28);
	keywords.set(L"iff", 29);
	keywords.set(L"otherwise", 30);
	keywords.set(L"for", 31);
	keywords.set(L"whilst", 32);
...
Token* Scanner::NextToken() {
	...
	switch (state) {
		...
		case 19:
			...
			if ((ch >= L'0' && ch <= L'9') || (ch >= L'A' && ch <= L'Z') || ch == L'_' || (ch >= L'a' && ch <= L'z')) {AddCh(); goto case_19;}
			else {t->kind = 21; wchar_t *literal = coco_string_create(tval, 0, tlen); t->kind = keywords.get(literal, t->kind); coco_string_delete(literal); break;}
		case 20:
			case_20:
			{t->kind = 23; break;}
		case 21:
		...
		case 30:
			...
			else if (ch == L'd') {AddCh(); goto case_21;}
			else {goto case_0;}
